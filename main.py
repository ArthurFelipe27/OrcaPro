...
def init_db():
    ...
    # Migração para quem já tem a tabela antiga
    try:
        c.execute('ALTER TABLE settings ADD COLUMN pdf_save_path TEXT')
    except sqlite3.OperationalError: pass
    
    try:
        c.execute('ALTER TABLE settings ADD COLUMN pdf_create_subfolder INTEGER')
    except sqlite3.OperationalError: pass

    # Migração orçamentos (garantir que colunas novas existam)
    new_cols = [('client_email', 'TEXT'), ('client_phone', 'TEXT'), ('client_address', 'TEXT')]
    for col, tipo in new_cols:
        try:
            c.execute(f'ALTER TABLE budgets ADD COLUMN {col} {tipo}')
        except sqlite3.OperationalError: pass
...
class ModernPDF(FPDF):
    def __init__(self, company_name, budget_id, date_str):
        super().__init__()
    ...
    def header(self):
        ...
        # Detalhes do Orçamento (Direita)
        self.set_font('Helvetica', '', 10)
        self.set_xy(10, 10)
        self.cell(0, 10, f"Orçamento #{self.budget_id}", 0, 1, 'R')
        self.cell(0, 5, f"Data: {self.date_str}", 0, 1, 'R')
...
class Api:
    def __init__(self):
        self.window = None # Referência à janela para abrir diálogos
    ...
    def save_settings(self, data):
        conn = sqlite3.connect(DB_FILE)
        c = conn.cursor()
        # Verificar se existe registro 1
        ...
        if exists:
            c.execute('''
                UPDATE settings 
                SET company_name=?, footer_text=?, pdf_save_path=?, pdf_create_subfolder=?
                WHERE id=1
            ''', (data['company'], data['footer'], data.get('pdf_path', ''), subfolder_int))
        else:
            c.execute('''
                INSERT INTO settings (id, company_name, footer_text, pdf_save_path, pdf_create_subfolder)
                VALUES (1, ?, ?, ?, ?)
            ''', (data['company'], data['footer'], data.get('pdf_path', ''), subfolder_int))
    ...
    def get_settings(self):
        ...
        if row:
            return {
                ...
                'pdf_path': row['pdf_save_path'] if 'pdf_save_path' in row.keys() else '',
                'create_subfolder': bool(row['pdf_create_subfolder']) if 'pdf_create_subfolder' in row.keys() else False
            }
        return {'company': '', 'footer': '', 'pdf_path': '', 'create_subfolder': False}
    ...
    def generate_pdf(self, budget_id):
        try:
            ...
            # Configurações
            company_name = settings['company_name'] if settings and settings['company_name'] else "Minha Empresa"
            footer_text = settings['footer_text'] if settings and settings['footer_text'] else ""
            save_path = settings['pdf_save_path'] if settings and settings['pdf_save_path'] else os.path.expanduser("~/Documents")
            use_subfolder = settings['pdf_create_subfolder'] if settings and 'pdf_create_subfolder' in settings.keys() else False

            # Validar caminho base, senão usar pasta atual
            ...
            # --- GERAÇÃO DO PDF ---
            pdf = ModernPDF(company_name, budget['id'], budget['date_created'])
            pdf.alias_nb_pages()
            pdf.add_page()
            ...
            # Larguras: Desc, Qtd, Unit, Total
            # Total largura A4 (210) - Margens (10+10) = 190
            ...
            for item in items:
                ...
                # Usa multi_cell fake para calcular altura
                line_height = 6
                ...
                # Escreve a descrição (que pode quebrar linha)
                pdf.multi_cell(w_desc, line_height, desc, border='LTB', align='L', fill=True)
                
                # Nova posição Y após escrever descrição
                y_after = pdf.get_y()
                row_height = y_after - y_before
                
                # Voltar para o topo da linha e desenhar as outras colunas com a altura da descrição
                ...
                pdf.ln() # Vai para a próxima linha baseada no maior height
            ...
            pdf.set_x(210 - 10 - width_total) # Margem direita
...
if __name__ == '__main__':
    ...
    # Passa a referência da janela para a API (para abrir diálogos)
    api.window = window
